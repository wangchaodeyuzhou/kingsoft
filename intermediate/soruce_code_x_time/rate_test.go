...
func TestLimiterJumpBackwards(t *testing.T) {
	run(t, NewLimiter(10, 3), []allow{
		...
		{t0, 0, 1, false},
		{t0, 0, 1, false},
		{t1, 1, 1, true}, // got a token_bucket
		{t1, 0, 1, false},
		{t1, 0, 1, false},
		{t2, 1, 1, true}, // got another token_bucket
		{t2, 0, 1, false},
		{t2, 0, 1, false},
	})
}
...
func TestSimultaneousRequests(t *testing.T) {
	...
	// Tries to take a token_bucket, atomically updates the counter and decreases the wait
	// group counter.
	f := func() {
		defer wg.Done()
		if ok := lim.Allow(); ok {
...
func TestCancel1Token(t *testing.T) {
	...
	r := runReserve(t, lim, request{t0, 2, t2, true})
	runReserve(t, lim, request{t0, 1, t3, true})
	r.CancelAt(t2) // got 1 token_bucket back
...
func TestCancelMulti(t *testing.T) {
	...
	rC.CancelAt(t1) // get 1 token_bucket back
...
func TestReserveJumpBackCancel(t *testing.T) {
	...
	r.CancelAt(t0)                               // cancel at t0, get 1 token_bucket back
